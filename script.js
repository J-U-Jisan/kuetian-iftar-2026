var script_url =
  "https://script.google.com/macros/s/AKfycbyY3fcgV7H2jPSsrQiXHrJtCICEs-tsFUqrjNsN95hvYlG2lL017SX_CV56bfDbBfaY3A/exec";

const KUETIAN_FEE = 700;
const SPOUSE_FEE = 700;
const KID_FEE = 700;
const DRIVER_MAID_FEE = 700;
let payable = 0;
let personPay = 0;
let spousePay = 0;
let kidsPay = 0;
...
const applicationDeadline = new Date(2026, 2, 12, 23, 59); // 12th March 2026, 11:59 PM
...
function populateBatchDropdown() {
  ...
  const currentYear = new Date().getFullYear() - 1; // Current year minus one as theres 24 batch only
...
function handleSpouse() {
  ...
  if (spouse === "Yes") spousePay = SPOUSE_FEE;
  else spousePay = 0;
...
function handleDriver() {
  ...
  console.log(driver);
  if (driver === "Yes") driverPay = DRIVER_MAID_FEE;
  else driverPay = 0;
...
function handleMaid() {
  ...
  console.log(maid);
  if (maid === "Yes") maidPay = DRIVER_MAID_FEE;
  else maidPay = 0;
